...
class randomization(rr.smooth_atom):
    ...
    def smooth_objective(self, perturbation, mode='both', check_feasibility=False):
        ...
        if mode == 'func':
            ...
            return self.scale(-np.log(self._density(perturbation))), self.scale(
                self._grad_negative_log_density(perturbation))
        else:
    ...
    def gaussian(covariance):
        ...
        return randomization((p,),
                             ...
                             log_density=lambda x: -np.sum(sqrt_precision.dot(np.atleast_2d(x).T) ** 2,
                                                           0) * 0.5 - np.log(_const),
    ...
    def degenerate_gaussian(covariance, tol=1.e-6):
        ...
        return randomization((p,),
                             ...
                             log_density=lambda x: -np.sum(sqrt_precision.T.dot(np.atleast_2d(x).T) ** 2,
                                                           0) * 0.5 - np.log(_const),
                             cov_prec=(covariance, precision))
    ...
    def logistic(shape, scale):
        ...
        return randomization(shape,
                             ...
                             log_density=lambda x: -np.atleast_2d(x).sum(1) / scale - 2 * np.log(
                                 1 + np.exp(-np.atleast_2d(x) / scale)).sum(1) + constant)
